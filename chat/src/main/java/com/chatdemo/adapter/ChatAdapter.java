...
import com.chatdemo.ChatActivity;
...
import com.chatdemo.bean.MsgBody;
...
import com.chatdemo.utils.GlideUtils;

import java.io.File;
import java.util.List;

public class ChatAdapter extends BaseQuickAdapter<MyMessage, BaseViewHolder> {
    ...
    public ChatAdapter(Context context, List<MyMessage> data) {
        super(data);
        setMultiTypeDelegate(new MultiTypeDelegate<MyMessage>() {
            @Override
            protected int getItemType(MyMessage entity) {
    ...
    private void setStatus(BaseViewHolder helper, MyMessage item) {
        MsgBody msgContent = item.getBody();
        if (msgContent instanceof TextMsgBody || msgContent instanceof AudioMsgBody) {
            //只需要设置自己发送的状态 ||msgContent instanceof VideoMsgBody  ||msgContent instanceof FileMsgBody
    ...
    private void setContent(BaseViewHolder helper, MyMessage item) {
    ...
    private void setOnClick(BaseViewHolder helper, MyMessage item) {
        MsgBody msgContent = item.getBody();
